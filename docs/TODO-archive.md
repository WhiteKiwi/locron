...
- [x] Record `locron-engine` as daemon-runtime owner and `locron` as the thin entrypoint exposing `locron daemon run` in the single v1 binary.
...
- [x] Create the Rust edition 2024, resolver-3 virtual workspace with exactly `locron-core`, `locron-store`, `locron-engine`, and `locron`.
...
- [x] Complete the per-option help surface: every argument of every command renders a concise
  ...
  **Verify:** a unit test walks the generated Clap command tree and asserts every non-hidden
  argument has non-empty help text; `cargo test -p locron` passes, and manual inspection of
...
- [x] Implement top-level `-V/--version` handling in `crates/locron-cli` with
  ...
  locron`, `cargo fmt --check`, and `cargo clippy -p locron` pass; manual runs show
...
- [x] Add contract tests in `crates/locron-cli/tests/version.rs`. **Verify:** `cargo test -p
  locron` passes, covering human plain text; `--format json` and `--json` envelopes
  ...
  **Evidence:** 114 `locron` tests pass, including the 10 new `version.rs` contract tests.
...
- [x] Implement `locron self-update` in `locron` (deps `sha2`, `tar`, `flate2`, plus `reqwest` rustls/stream/json as the TLS client; `LOCRON_UPDATE_API_BASE`/`LOCRON_UPDATE_ASSET_BASE` test seams): latest resolution, checksum verification, atomic self-replace, marker refusal (`lib/.disable-self-update`), human and `locron.cli/v1` output, stable error mapping.
  **Verify:** `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`, and `cargo test -p locron` pass; the real-API smoke `locron --json self-update` returns the `locron.cli/v1` envelope with `current_version`/`new_version`/`updated` and exit 0. The reqwest addition is recorded in `docs/IMPLEMENTATION.md`.
...
- [x] Add the service-manager port to `locron`: launchd and systemd-user backends behind a shared port plus a deterministic fake; embed the plist/unit templates (label `dev.locron.daemon`, `locron.service`, canonicalized `current_exe` path, `KeepAlive`/`RunAtLoad`, `Restart=on-failure`/`WantedBy=default.target`, `~/Library/Logs/locron/daemon.log`).
  **Verify:** unit tests render both templates with a canonicalized binary path and the required keys; `cargo fmt --check` and `cargo clippy -p locron` pass; workspace inspection still finds only the `locron` binary and no new dependency.
  **Evidence:** 15 unit tests in `service::tests` pass, including `plist_template_renders_required_keys_and_paths` and `unit_template_renders_required_keys_and_paths`; `cargo fmt --all --check` and `cargo clippy -p locron --all-targets -- -D warnings` pass; `locron-engine`/`locron-store` Cargo.toml unchanged (no new dependency).
- [x] Implement `locron service install|uninstall|status` over the port with human and `locron.cli/v1` output, brew-marker refusal, lock-held deferral, and no-session guidance (exit 0).
  ...
  **Evidence:** 11 contract tests in `tests/service.rs` pass (install ordering, refresh restart, flock deferral, uninstall ordering, status fields, guidance exit 0 human+JSON, marker refusal exit 3 `service_managed_install`, forced-unsupported exit 2, help text); `cargo test -p locron` fully green.
...
- [x] Gate the launchd-only items so Linux builds compile clean: `LABEL`, `LOG_DIR`, `LOG_FILE`, `escape_xml`, and `render_plist` under `#[cfg(any(target_os = "macos", test))]` (mirroring the existing `render_unit` pattern; the plist unit tests run on every platform), `PLIST_NAME` under `#[cfg(target_os = "macos")]`, the launchd-only `ServiceContext.uid` field under a documented targeted `allow(dead_code)`, the unused `Path` import and the `map().unwrap_or_else()` in the systemd module fixed, and the PathBuf `Display` uses in `tests/service_backends.rs` switched to `.display()`.
  ...
  **Verify:** `cargo fmt --all --check`, `cargo clippy -p locron --all-targets -- -D warnings`, and `cargo test -p locron` pass on macOS; the four-target CI matrix passes on push — run 32644269125 (head 7edc89d), all nine jobs green.
...
and URL import implementation"; evidence in `docs/FINDINGS.md` §15. Confined to `locron`;
...
- [x] Run full workspace verification and record evidence, then mark this backlog complete.
  ...
  **Evidence:** on Rust 1.94.0 (the workspace MSRV; the installed toolchain is 1.94.0), `cargo fmt
  ...
  test --workspace` passes 315 tests with 0 failures (locron: 67 unit + 50 contract). The
...
- [x] Add `#[command(visible_alias = "ls")]` to the `List` variant and
  ...
  **Verify:** `cargo fmt --all --check`, `cargo clippy -p locron --all-targets -- -D warnings`,
  and `cargo test -p locron` pass; `locron ls --all` and `locron rm <name>` against a scratch
...
- [x] Replace the pretty-JSON human fallback for `list` only with an aligned table: header always
  ...
  **Verify:** `cargo fmt --all --check`, `cargo clippy -p locron --all-targets -- -D warnings`,
  and `cargo test -p locron` pass; manual runs against a scratch state directory show the
...
- [x] Add the thin `explain` command and shared current-job explanation path, including the bounded
  ...
  **Evidence:** `cargo fmt --all --check` and `cargo clippy -p locron --all-targets -- -D
...
- [x] Add CLI contract/integration coverage for no history, success-only history, the same latest
  ...
  **Evidence:** four new CLI contract tests cover every named state/history/removal/reuse scenario,
  ...
  `cargo test -p locron` passes 73 CLI unit tests, 72 command-contract tests, and all 13 other
...
This shipped post-milestone program was authorized by the frozen 2026-08-24
`docs/dashboard/SPEC.md` (interactive product review); planned in `docs/dashboard/IMPLEMENTATION.md`; evidence
in `docs/FINDINGS.md` §14 (including the default-port 10824 verification).
...
- [x] Add the `locron-server` member to the workspace with axum 0.8.9, tokio-stream 0.1.19,
  ...
  **Verify:** `cargo build`/`fmt --check`/`clippy -p locron-server` pass on Rust 1.94 and latest
  ...
  `locron-store` and nothing depends on it but `locron`; the workspace still produces exactly
  one `locron` binary.
  **Evidence:** `crates/locron-server` added as the fifth workspace member (library only — no
  ...
  serde/serde_json/tokio/uuid/base64/reqwest; dev-dep tempfile); `locron` gained the
  ...
  among workspace crates and that only `locron` depends on it) and wired it into the CI test
...
- [x] Wire the `locron dashboard [--port N] [--bind ADDR]` family (`serve` alias, `enable`,
  `disable`, `status`, `token`) in `locron`, add the doctor exposure facts, and extend the
...
- [x] Run full workspace verification and record evidence for the shipped dashboard program.
  **Verify:** `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`,
  and `cargo test --workspace` pass on Rust 1.94 and latest stable; the four-target CI matrix is
  green; the browser-checklist and real-backend evidence are recorded in this section, and the
  shipped contract and plan remain in `docs/dashboard/SPEC.md` and
  ...
  **Evidence (final gate, 2026-08-24):** on the final tree, `cargo fmt --all --check`,
  ...
  The shipped dashboard program is complete, evidenced by `docs/dashboard/SPEC.md` (frozen
...
- [x] Implement the table and confirmation-line renderers: `history` table, `add`/`update`,
  ...
  byte-identical.
  **Verify:** `cargo fmt --all --check`, `cargo clippy -p locron --all-targets -- -D
  warnings`, and `cargo test -p locron` pass; manual scratch-state runs show the documented
  forms for each command, and each `--json` output is unchanged.
  **Evidence:** `cargo fmt --all --check` is clean; `cargo clippy -p locron --all-targets --
  -D warnings` is clean; `cargo test -p locron` passes 198 tests (67 unit in main.rs, 65
...
- [x] Run workspace verification, then close issue #4 with the fixing commit reference.
  ...
  **Evidence:** `cargo fmt --all --check` is clean; `cargo clippy --workspace --all-targets --
  ...
  failures (locron: 67 unit + 65 contract + 66 across its other 12 test suites; the +15
...
- [x] Amend planning documents before code: the SPEC amendment and status note, the `docs/CLI.md`
  ...
  **Evidence:** the frozen SPEC table-width amendment, CLI `--no-trunc` contract,
  IMPLEMENTATION section, FINDINGS §19, and STATUS release record all exist and agree; the
  implementation and v0.5.0 publication evidence below confirm this stale planning checkbox was
  complete.
- [x] Implement width resolution (`console::Term::stdout().size_checked()`), `truncate_display`
  ...
  **Verify:** the new unit tests pass (`truncate_display` ASCII/CJK/emoji/boundary cases;
  ...
  locron --all-targets -- -D warnings`, and `cargo test -p locron` pass.
  **Evidence:** `cargo fmt --all --check` clean; `cargo clippy -p locron --all-targets --
  -D warnings` clean; `cargo test -p locron` 206 passed, 0 failed, including the new
  ...
  `unicode-width` edges under the `locron` package. Real-PTY check (`TIOCGWINSZ` via
