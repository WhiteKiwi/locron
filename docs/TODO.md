...
- [ ] Close the installer CI verification gaps: shellcheck, unsupported-platform/musl refusals, and the Linux install leg, via the new `installer` job in `.github/workflows/ci.yml`.
...
- [ ] Minor cleanup: the HOME-unset guard at the top of `install.sh` is dead code after the default expansion and can be removed. Deferred to the session currently editing `install.sh` (service-registration wiring) to avoid file conflicts.
