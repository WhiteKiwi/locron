...
- **Table width** — on a terminal, when a table would exceed the terminal width, `list` truncates `TARGET` and `history` truncates `TRIGGER`, marks truncation with a trailing ellipsis, and keeps every other column intact. When standard output is redirected or piped, no truncation occurs and every value prints in full, so scripts and automation always receive complete values. Each table's rendering flag restores full-width values on a terminal; the dedicated job detail report and machine-readable output also retain complete values. Column fitting uses character display width, never byte length. Machine-readable output is unaffected.
