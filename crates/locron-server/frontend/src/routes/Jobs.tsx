...
export function Jobs() {
  ...
  useEffect(() => {
    ...
    setError("");
    api.get<Job[]>("/api/v1/jobs?all=1", { signal: controller.signal }).then(({ data: jobs }) => setData(jobs)).catch((issue) => setError(issue.message));
  ...
  useEffect(() => {
    ...
    void Promise.all(data.map(async (job) => {
      const [nextResult, lastResult] = await Promise.allSettled([
        job.enabled ? api.get<{ occurrences: string[] }>(`/api/v1/jobs/${job.id}/preview?count=1`, { signal: controller.signal }) : Promise.resolve(null),
      ...
      const next = !job.enabled ? "disabled — not scheduled" : nextResult.status === "fulfilled" && nextResult.value ? (nextResult.value.data.occurrences[0] ? new Date(nextResult.value.data.occurrences[0]).toLocaleString() : "no future occurrence") : "unavailable";
  ...
  return <>
    ...
    {error ? <Feedback kind="error">{error}</Feedback> : data === null ? <div className="loading-state" aria-busy="true">Loading jobs…</div> : <ResponsiveData desktop={<div className="table-scroll"><table aria-describedby="jobs-results-status"><thead><tr><th>Job</th><th>Schedule</th><th>Next</th><th>Last run</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{rows.length ? rows.map((job) => <tr className="clickable-row" onClick={navigateRow} key={job.id}><td><a data-row-link href={`#/jobs/${encodeURIComponent(job.id)}`}>{job.name}<span className="sr-only"> — view job details</span></a><span className="sub">{tagsFor(job).join(" · ") || "no tags"}</span></td><td>{scheduleSummary(job)}<span className="sub"><StatusBadge status={job.enabled ? "enabled" : "disabled"}/></span></td><td>{job.enabled ? facts[job.id]?.next ?? "…" : "disabled — not scheduled"}</td><td>{facts[job.id]?.last ?? "…"}</td><td>{rowActions(job)}</td></tr>) : <EmptyTableRow columns={5} title={emptyTitle} description={emptyDescription} actions={emptyActions()} />}</tbody></table></div>} mobile={rows.length ? <div className="object-list">{rows.map((job) => <article className="object-row clickable-row" onClick={navigateRow} key={job.id}><div className="object-row-head"><div><a data-row-link className="object-title" href={`#/jobs/${encodeURIComponent(job.id)}`}>{job.name}<span className="sr-only"> — view job details</span></a><span className="sub">{tagsFor(job).join(" · ") || "no tags"}</span></div>{rowActions(job)}</div><dl><dt>Schedule</dt><dd>{scheduleSummary(job)}</dd><dt>State</dt><dd><StatusBadge status={job.enabled ? "enabled" : "disabled"}/></dd><dt>Next</dt><dd>{job.enabled ? facts[job.id]?.next ?? "…" : "disabled — not scheduled"}</dd><dt>Last run</dt><dd>{facts[job.id]?.last ?? "…"}</dd></dl><button type="button" className="inline-action" onClick={() => void act(job,"run")}><Play size={15}/>Run now</button></article>)}</div> : <EmptyObjectList title={emptyTitle} description={emptyDescription} actions={emptyActions()} />} />}
  </>;
}
